...
void RemoteCompactionPrimaryCommand::Help(std::string& ret) {
  ...
  ret.append("\n    Create DB, flush, run CompactRange() through ");
...
void RemoteCompactionPrimaryCommand::DoCommand() {
  ...
  // Two levels guarantees CompactRange() produces exactly one L0→L1 job,
  // so the file-based protocol only sees one Schedule()/Wait() cycle.
  options.num_levels = 2;

  DestroyDB(db_path_, options).PermitUncheckedError();
  ...
  for (int f = 0; f < 4; f++) {
    for (int i = 0; i < 20; i++) {
      s = db->Put(WriteOptions(), "key_" + std::to_string(f * 20 + i),
                  std::string(100, 'a' + (i % 26)));
      if (!s.ok()) {
        exec_state_ = LDBCommandExecuteResult::Failed("Put: " + s.ToString());
        return;
      }
    }
    s = db->Flush(FlushOptions());
    if (!s.ok()) {
      exec_state_ = LDBCommandExecuteResult::Failed("Flush: " + s.ToString());
      return;
    }
  }

  // CompactRange() calls Schedule() → writes input.bin → calls Wait() →
  // polls for result.bin → CompactionServiceResult::Read() → installs.
