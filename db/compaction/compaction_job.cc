...
void CompactionJob::Prepare(
  ...
  if (compact_->sub_compact_states.size() == 1) {
    resumable_compaction_progress_writer_ =
...
Status CompactionJob::FinishCompactionOutputFile(
  ...
  TEST_SYNC_POINT_CALLBACK(
      ...
      nullptr);
  if (s.ok() && ShouldUpdateResumableSubcompactionProgress(
                    sub_compact, prev_table_last_internal_key,
...
bool CompactionJob::ShouldUpdateResumableSubcompactionProgress(
  ...
  // TODO: depend on manifest writer

  // No need to save empty output
  if (meta == nullptr) {
    return false;
  }
...
Status CompactionJob::MaybeResumeSubcompactionProgressOnInputIterator(
  ...
  if (resumable_subcompaction_progress.num_processed_input_records == 0) {
...
Status CompactionJob::PersistResumableSubcompactionProgress(
  ...
  // Skip persistence if no writer is available
  if (resumable_compaction_progress_writer_ == nullptr) {
    ROCKS_LOG_INFO(
        db_options_.info_log,
        "[%s] [JOB %d] No progress writer available, skipping "
        "resumable compaction progress persistence",
        compact_->compaction->column_family_data()->GetName().c_str(), job_id_);
    return Status::OK();
  }

  // Create VersionEdit with the current resumable subcompaction progress
  VersionEdit edit;
  ...
  if (!edit.EncodeTo(&record)) {
    ROCKS_LOG_ERROR(
        db_options_.info_log,
        "[%s] [JOB %d] Failed to encode resumable compaction "
        "progress to VersionEdit",
        compact_->compaction->column_family_data()->GetName().c_str(), job_id_);
    return Status::IOError("Failed to encode resumable compaction progress");
  }

  Status s =
      resumable_compaction_progress_writer_->AddRecord(WriteOptions(), record);
  if (s.ok()) {
    s = resumable_compaction_progress_writer_->file()->Sync(IOOptions(), false);
  }

  if (!s.ok()) {
    ROCKS_LOG_ERROR(
        db_options_.info_log,
        "[%s] [JOB %d] Failed to persist resumable compaction "
        "progress: %s",
        compact_->compaction->column_family_data()->GetName().c_str(), job_id_,
        s.ToString().c_str());
    return s;
