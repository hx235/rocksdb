      ...
      if (use_txn) {
        ...
        if (thread->rand.OneIn(10)) {
          ...
          switch (op) {
            case 0:
            case 1: {
              // No need to use `ExpectedValueHelper::NextValueBase()` here since the
              // value generated by this `value_base` won't be committed to db
              // as `txn` containing this value will be rolled back
    ...
    if (s.ok()) {
      ...
      if (value_from_db.length() != expected_value_data_size) {
        VerificationAbort(shared,
                          ...
                          cf, key, value_from_db, Slice(expected_value_data, expected_value_data_size));
        return false;
      }
