...
const std::string kResumableCompactionProgressFileNamePrefix =
...
std::string ResumableCompactionProgressFileName(const std::string& dbname,
                                                uint64_t timestamp) {
  char buffer[256];
  snprintf(buffer, sizeof(buffer), "%s%llu",
           kResumableCompactionProgressFileNamePrefix.c_str(),
           static_cast<unsigned long long>(timestamp));
  return dbname + "/" + buffer;
}

std::string TempResumableCompactionProgressFileName(const std::string& dbname,
                                                    uint64_t timestamp) {
  char buffer[256];
...
//    dbname/RESUMABLE_COMPACTION_PROGRESS-[timestamp]
//    dbname/RESUMABLE_COMPACTION_PROGRESS-[timestamp].dbtmp
...
bool ParseFileName(const std::string& fname, uint64_t* number,
  ...
  if (rest == "IDENTITY") {
  ...
  } else if (rest.starts_with(kResumableCompactionProgressFileNamePrefix)) {
    uint64_t timestamp;
    bool is_temp_file = false;
    rest.remove_prefix(kResumableCompactionProgressFileNamePrefix.size());
    const std::string kTempFileNameSuffixWithDot =
        std::string(".") + kTempFileNameSuffix;
    if (rest.ends_with(kTempFileNameSuffixWithDot)) {
      rest.remove_suffix(kTempFileNameSuffixWithDot.size());
    ...
    *number = timestamp;
    *type = is_temp_file ? kTempFile : kResumableCompactionProgressFile;
  } else {
